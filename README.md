...
Das hier ist mein Docker Image für Tomcat (10.1.12 mit Temurin 17).
...
```dockerfile
FROM ghcr.io/schipplock/tomcat-docker-image:v10.1.12
...
```bash
docker build --no-cache --network=host --force-rm -t ghcr.io/schipplock/tomcat-docker-image:v10.1.12 .
```